...
# Theoretical line with -5/4 slope
theoretical_line = -1.25 * ln_k + coeffs[1]  # Same intercept as fitted

print(f"Fitted slope: {coeffs[0]:.4f}")
print(f"Theoretical: -1.25")
print(f"Difference: {abs(coeffs[0] + 1.25):.4f}")
...
plt.plot(ln_k, theoretical_line, 'g--', linewidth=1.5, 
         label='Theoretical: -5/4 ln(k)')
...
plt.savefig('./figures/residuals_verification.pdf')
plt.show()
